...
namespace esphome {
	namespace ble_mi_remote {
		...
		void BleMiRemote::setup() {
			...
			NimBLEService* sVendor_6287 = hid->hidService()->createService((uint16_t) 0x6287);
			...
			NimBLEService* sVendor_d1ff = hid->hidService()->createService(uint16_t) 0xd1ff);
			...
			NimBLEService* sVendor_d0ff = hid->hidService()->createService(uint16_t) 0xd0ff);
